...
exports.defaultTemplateVars = (req) => {
  ...
  return { 
      ...
      successcode : this.grabSuccessCode(req),
      register : false,
      user: db.users[req.session.userid]
  };
}

exports.grabSuccessCode = (req) => {
  if(req.query.s == 1) return 'added_earl';
  if(req.query.s == 2) return 'modified_earl';
  if(req.query.s == 3) return 'deleted_earl';
  return null;
}
